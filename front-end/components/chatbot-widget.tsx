  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const botMessage: Message = {
        ...
        text: data.response, // Uses the 'response' key from your FastAPI return
        sender: 'bot',
        timestamp: new Date(),
      }
  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed bottom-4 right-4 z-40 w-14 h-14 rounded-full bg-gradient-to-br from-primary to-accent shadow-lg hover:shadow-xl transition-all duration-300 flex items-center justify-center text-primary-foreground ${
      ...
      {/* Chat Window */}
      {isOpen && (
        <div className="fixed bottom-4 right-4 z-40 w-96 max-w-[calc(100vw-32px)] h-[600px] max-h-[70vh] rounded-2xl bg-card shadow-2xl border border-border flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-300">
          {/* Header */}
